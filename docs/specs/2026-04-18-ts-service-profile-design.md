...
### Tag model clarification

`cli/v1.6.0` is a single repo-wide git tag that points to the merge commit of this PR. The tag references the entire repo state at that commit, which includes:
- The reusable workflows (`reusable-pr-gate-typescript.yml`, `reusable-pr-gate-python.yml`) as they exist on main when v1.6.0 is tagged.
- The new profile files and templates introduced in this PR.
- The Python CLI source at `__version__ = "1.6.0"`.

There is no separate versioning for the reusable workflow — it inherits the repo's tag. Therefore `uses: yakkuro/gh-manage/.github/workflows/reusable-pr-gate-typescript.yml@v1.6.0` resolves to the reusable workflow's state at the v1.6.0 commit, which is functionally identical to v1.5.0's reusable workflow (no workflow YAML changes in this PR). Future PRs that bump the reusable workflow will bundle that change with whatever cli/vN.N tag ships next — the tag is the single source of truth for "what does @vN.N.N resolve to".
...
```
Expected scan-level outcome: **22 repos scanned, 0 FAILED**. "FAILED" in `drift --all`'s summary means a repo-level scan error (profile load failure, network issue, auth failure) — not the severity of per-repo findings.

Expected finding-level outcome (new after this PR): `yakkuro/nade-nade` reports a drift **finding** on `profile_files/.github/workflows/ci.yml` with severity `medium` because the repo's current `ci.yml` diverges from the new `ts-service` template (still pins the Python reusable workflow). This is the intended behavior — the finding is what triggers Phase 2 migration. It appears on the first `drift --all` run after merge (not waiting for cron).
...
- `tests/test_sanity.py` — assertion updated to `1.6.0`
- `uv.lock` — regenerated by `uv sync` after the version bump
...
- [ ] Self-dogfood `uv run gh-manage drift --all` reports 22 repos scanned, 0 FAILED (scan-level errors); the new nade-nade `profile_files/.github/workflows/ci.yml` finding is expected and intentional (Phase 2 trigger).
...
Spec-critique round 1 findings addressed:
- **HIGH** (reusable workflow v1.6.0 coordination): §3 expanded with "Tag model clarification" — single repo-wide tag, no separate reusable-workflow versioning.
- **MEDIUM** (drift "0 FAILED" vs findings ambiguity): §5 Self-dogfood + §8 acceptance criteria clarified. FAILED = scan-level errors, not finding severity. The new nade-nade ci.yml finding is intentional.
- **MEDIUM / LOW** (uv.lock in release list): added to §6 Files bumped.

Rejected (YAGNI or out of scope):
- "workflows_version field in profile YAML" (Codex MEDIUM) — python-service doesn't have it; adding it here creates asymmetry. If profile-to-workflow version tracking becomes a real concern, open a separate follow-up for both profiles.
- "reusable workflow pnpm optimization" (Claude MEDIUM) — explicitly out of scope per §Non-goals; the overhead is documented as acceptable.
- "manual CLAUDE.md verification" (Codex LOW) — `default.md` content was read during brainstorming; contains no Python-specific references (sections: overview, tech stack, conventions, references — all language-agnostic).
